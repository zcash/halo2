...
impl<F: FieldExt> Arity3Chip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> Arity3Config<F> {
        // Generate constants for the Poseidon permutation.
        // TODO
        let half_full_rounds = 4;
        let half_partial_rounds = 28;
        let round_constants = vec![[F::one(); ARITY]; 2 * (half_full_rounds + half_partial_rounds)];
        let m_reg = [[F::one(); ARITY]; ARITY];
        let m_inv = [[F::one(); ARITY]; ARITY];
        ...
        meta.create_gate("partial round state_0 part_b", |meta| {
            ...
            s_partial
                * (mid_0 * m_reg[2][0]
                    + (cur_1 + rc_a1) * m_reg[2][1]
                    + (cur_2 + rc_a2) * m_reg[2][2]
                    + rc_b2
                    - (next_0 * m_inv[2][0] + next_1 * m_inv[2][1] + next_2 * m_inv[2][2]))
        });
...
impl<F: FieldExt> Arity3State<F> {
    fn full_round(
        ...
        Self::round(region, config, round, offset, config.s_full, |_| {
            ...
            let r_0 = q_0.map(|v| v.pow(ALPHA));
            let r_1 = q_1.map(|v| v.pow(ALPHA));
            let r_2 = q_2.map(|v| v.pow(ALPHA));
    ...
    fn partial_round(
        ...
        Self::round(region, config, round, offset, config.s_partial, |region| {
            ...
            let r = p.map(|p| {
                [
                    (p[0] + config.round_constants[round][0]).pow(ALPHA),
            ...
            let r_mid = p_mid.map(|p| {
                [
                    (p[0] + config.round_constants[round + 1][0]).pow(ALPHA),
    ...
    fn final_round(
        ...
        Self::round(region, config, round, offset, config.s_final, |_| {
            let mut new_state = self
                ...
                .map(|(word, rc)| word.value.map(|v| (v + rc).pow(ALPHA)));
...
mod tests {
    ...
    fn print_poseidon_chip() {
        ...
        struct MyConfig<F: FieldExt> {
            poseidon: Arity3Config<F>,
        }

        struct MyCircuit {}

        impl Circuit<Fp> for MyCircuit {
            type Config = MyConfig<Fp>;

            fn configure(meta: &mut ConstraintSystem<Fp>) -> MyConfig<Fp> {
                let poseidon = Arity3Chip::configure(meta);
                MyConfig { poseidon }
            }
