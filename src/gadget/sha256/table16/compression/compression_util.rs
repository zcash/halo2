...
impl Compression {
    pub(super) fn decompose_abcd<F: FieldExt>(
        ...
        Error,
    > {
        region.assign_fixed(self.s_decompose_abcd, row, || Ok(F::one()))?;
    ...
    pub(super) fn decompose_efgh<F: FieldExt>(
        ...
        Error,
    > {
        region.assign_fixed(self.s_decompose_efgh, row, || Ok(F::one()))?;
    ...
    pub(super) fn assign_upper_sigma_0<F: FieldExt>(
        ...
        region.assign_fixed(self.s_upper_sigma_0, row, || Ok(F::one()))?;

        // Assign `spread_a` and copy constraint
        self.assign_and_constrain(region, a_3, row + 1, &word.a.spread, &self.perm)?;
        // Assign `spread_b` and copy constraint
        self.assign_and_constrain(region, a_5, row, &word.b.spread, &self.perm)?;
        // Assign `spread_c_lo` and copy constraint
        self.assign_and_constrain(region, a_3, row - 1, &word.c_lo.spread, &self.perm)?;
        // Assign `spread_c_mid` and copy constraint
        self.assign_and_constrain(region, a_4, row - 1, &word.c_mid.spread, &self.perm)?;
        // Assign `spread_c_hi` and copy constraint
        self.assign_and_constrain(region, a_4, row + 1, &word.c_hi.spread, &self.perm)?;
        // Assign `spread_d` and copy constraint
        self.assign_and_constrain(region, a_4, row, &word.d.spread, &self.perm)?;
    ...
    pub(super) fn assign_upper_sigma_1<F: FieldExt>(
        ...
        region.assign_fixed(self.s_upper_sigma_1, row, || Ok(F::one()))?;

        // Assign `spread_a_lo` and copy constraint
        self.assign_and_constrain(region, a_3, row + 1, &word.a_lo.spread, &self.perm)?;
        // Assign `spread_a_hi` and copy constraint
        self.assign_and_constrain(region, a_4, row + 1, &word.a_hi.spread, &self.perm)?;
        // Assign `spread_b_lo` and copy constraint
        self.assign_and_constrain(region, a_3, row - 1, &word.b_lo.spread, &self.perm)?;
        // Assign `spread_b_hi` and copy constraint
        self.assign_and_constrain(region, a_4, row - 1, &word.b_hi.spread, &self.perm)?;
        // Assign `spread_c` and copy constraint
        self.assign_and_constrain(region, a_5, row, &word.c.spread, &self.perm)?;
        // Assign `spread_d` and copy constraint
        self.assign_and_constrain(region, a_4, row, &word.d.spread, &self.perm)?;
    ...
    pub(super) fn assign_ch<F: FieldExt>(
        ...
        region.assign_fixed(self.s_ch, row, || Ok(F::one()))?;

        // Assign and copy spread_e_lo, spread_e_hi
        self.assign_and_constrain(region, a_3, row - 1, &spread_halves_e.0, &self.perm)?;
        self.assign_and_constrain(region, a_4, row - 1, &spread_halves_e.1, &self.perm)?;

        // Assign and copy spread_f_lo, spread_f_hi
        self.assign_and_constrain(region, a_3, row + 1, &spread_halves_f.0, &self.perm)?;
        self.assign_and_constrain(region, a_4, row + 1, &spread_halves_f.1, &self.perm)?;
    ...
    pub(super) fn assign_ch_neg<F: FieldExt>(
        ...
        region.assign_fixed(self.s_ch_neg, row, || Ok(F::one()))?;
        ...
        // Assign and copy spread_e_lo, spread_e_hi
        self.assign_and_constrain(region, a_5, row - 1, &spread_halves_e.0, &self.perm)?;
        self.assign_and_constrain(region, a_5, row, &spread_halves_e.1, &self.perm)?;

        // Assign and copy spread_g_lo, spread_g_hi
        self.assign_and_constrain(region, a_3, row + 1, &spread_halves_g.0, &self.perm)?;
        self.assign_and_constrain(region, a_4, row + 1, &spread_halves_g.1, &self.perm)?;
        ...
        region.assign_advice(a_3, row - 1, || Ok(F::from_u64(spread_neg_e_lo)))?;
        region.assign_advice(a_4, row - 1, || Ok(F::from_u64(spread_neg_e_hi)))?;
    ...
    pub(super) fn assign_maj<F: FieldExt>(
        ...
        region.assign_fixed(self.s_maj, row, || Ok(F::one()))?;

        // Assign and copy spread_a_lo, spread_a_hi
        self.assign_and_constrain(region, a_4, row - 1, &spread_halves_a.0, &self.perm)?;
        self.assign_and_constrain(region, a_5, row - 1, &spread_halves_a.1, &self.perm)?;

        // Assign and copy spread_b_lo, spread_b_hi
        self.assign_and_constrain(region, a_4, row, &spread_halves_b.0, &self.perm)?;
        self.assign_and_constrain(region, a_5, row, &spread_halves_b.1, &self.perm)?;

        // Assign and copy spread_c_lo, spread_c_hi
        self.assign_and_constrain(region, a_4, row + 1, &spread_halves_c.0, &self.perm)?;
        self.assign_and_constrain(region, a_5, row + 1, &spread_halves_c.1, &self.perm)?;
    ...
    pub(super) fn assign_h_prime<F: FieldExt>(
        ...
        region.assign_fixed(self.s_h_prime, row, || Ok(F::one()))?;
        ...
        // Assign and copy h
        self.assign_and_constrain(region, a_7, row - 1, &h.0.into(), &self.perm)?;
        self.assign_and_constrain(region, a_7, row, &h.1.into(), &self.perm)?;

        // Assign and copy sigma_1
        self.assign_and_constrain(region, a_4, row, &sigma_1.0.into(), &self.perm)?;
        self.assign_and_constrain(region, a_5, row, &sigma_1.1.into(), &self.perm)?;

        // Assign k
        let k_pieces = chop_u32(k, &[16, 16]);
        region.assign_advice(a_6, row - 1, || Ok(F::from_u64(k_pieces[0] as u64)))?;
        region.assign_advice(a_6, row, || Ok(F::from_u64(k_pieces[1] as u64)))?;

        // Assign and copy w
        self.assign_and_constrain(region, a_8, row - 1, &w.0.into(), &self.perm)?;
        self.assign_and_constrain(region, a_8, row, &w.1.into(), &self.perm)?;

        // Assign and copy ch
        self.assign_and_constrain(region, a_6, row + 1, &ch.1.into(), &self.perm)?;

        // Assign and copy ch_neg
        self.assign_and_constrain(region, a_5, row - 1, &ch_neg.0.into(), &self.perm)?;
        self.assign_and_constrain(region, a_5, row + 1, &ch_neg.1.into(), &self.perm)?;
        ...
        let h_prime_lo =
            region.assign_advice(a_7, row + 1, || Ok(F::from_u64(h_prime_halves[0] as u64)))?;
        let h_prime_hi =
            region.assign_advice(a_8, row + 1, || Ok(F::from_u64(h_prime_halves[1] as u64)))?;
        region.assign_advice(a_9, row + 1, || Ok(F::from_u64(h_prime_carry as u64)))?;
    ...
    pub(super) fn assign_e_new<F: FieldExt>(
        ...
        region.assign_fixed(self.s_e_new, row, || Ok(F::one()))?;
        ...
        self.assign_and_constrain(region, a_7, row, &d.0.into(), &self.perm)?;
        self.assign_and_constrain(region, a_7, row + 1, &d.1.into(), &self.perm)?;
        ...
        region.assign_advice(a_9, row + 1, || Ok(F::from_u64(e_new_carry as u64)))?;
    ...
    pub(super) fn assign_a_new<F: FieldExt>(
        ...
        region.assign_fixed(self.s_a_new, row, || Ok(F::one()))?;
        ...
        self.assign_and_constrain(region, a_3, row - 1, &maj.1.into(), &self.perm)?;

        // Assign and copy sigma_0
        self.assign_and_constrain(region, a_6, row, &sigma_0.0.into(), &self.perm)?;
        self.assign_and_constrain(region, a_6, row + 1, &sigma_0.1.into(), &self.perm)?;

        // Assign and copy h_prime
        self.assign_and_constrain(region, a_7, row - 1, &h_prime.0.into(), &self.perm)?;
        self.assign_and_constrain(region, a_8, row - 1, &h_prime.1.into(), &self.perm)?;
        ...
        region.assign_advice(a_9, row, || Ok(F::from_u64(a_new_carry as u64)))?;
    ...
    pub fn assign_word_halves_dense<F: FieldExt>(
        ...
        let lo = region.assign_advice(lo_col, lo_row, || Ok(F::from_u64(halves[0] as u64)))?;
        let hi = region.assign_advice(hi_col, hi_row, || Ok(F::from_u64(halves[1] as u64)))?;
