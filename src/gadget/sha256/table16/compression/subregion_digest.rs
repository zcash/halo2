...
impl Compression {
    pub fn assign_digest<F: FieldExt>(
        ...
        let abcd_row = 0;
        region.assign_fixed(self.s_digest, abcd_row, || Ok(F::one()))?;
        let efgh_row = abcd_row + 2;
        region.assign_fixed(self.s_digest, efgh_row, || Ok(F::one()))?;

        // Assign digest for A, B, C, D
        self.assign_and_constrain(region, a_3, abcd_row, &a.dense_halves.0.into(), &self.perm)?;
        self.assign_and_constrain(region, a_4, abcd_row, &a.dense_halves.1.into(), &self.perm)?;
        let a = a.dense_halves.0.value.unwrap() as u32
            + (1 << 16) * (a.dense_halves.1.value.unwrap() as u32);
        region.assign_advice(a_5, abcd_row, || Ok(F::from_u64(a as u64)))?;
        ...
        self.assign_and_constrain(region, a_3, efgh_row, &e.dense_halves.0.into(), &self.perm)?;
        self.assign_and_constrain(region, a_4, efgh_row, &e.dense_halves.1.into(), &self.perm)?;
        let e = e.dense_halves.0.value.unwrap() as u32
            + (1 << 16) * (e.dense_halves.1.value.unwrap() as u32);
        region.assign_advice(a_5, efgh_row, || Ok(F::from_u64(e as u64)))?;
    ...
    fn assign_digest_word<F: FieldExt>(
        ...
        self.assign_and_constrain(region, lo_col, row, &dense_halves.0.into(), &self.perm)?;
        self.assign_and_constrain(region, hi_col, row, &dense_halves.1.into(), &self.perm)?;
        let val = dense_halves.0.value.unwrap() as u32
            + (1 << 16) * (dense_halves.1.value.unwrap() as u32);
        region.assign_advice(word_col, row, || Ok(F::from_u64(val as u64)))?;
