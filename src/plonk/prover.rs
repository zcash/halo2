...
>(
    ...
    let mut vanishing_proof = vanishing::Proof::default();
    let mut proof_instances = vec![InstanceProof::default(); circuits.len()];
    ...
    let advice: Vec<AdviceSingle<C>> = circuits
        .iter()
        .zip(instances.iter())
        .zip(proof_instances.iter_mut())
        .map(
            |((circuit, instances), proof)| -> Result<AdviceSingle<C>, Error> {
                ...
                let advice_blinds: Vec<_> =
                    advice.iter().map(|_| Blind(C::Scalar::rand())).collect();
                ...
                proof.advice_commitments = advice_commitments.clone();
    ...
    let lookups: Vec<Vec<lookup::prover::Permuted<C>>> = instance
        ...
        .zip(proof_instances.iter_mut())
        .map(
            |((instance, advice), instance_proof)| -> Result<Vec<_>, Error> {
                // Construct and commit to permuted values for each lookup
                pk.vk
                    .cs
                    .lookups
                    .iter()
                    .zip(instance_proof.lookups.iter_mut())
                    .map(|(lookup, mut lookup_proof)| {
    ...
    let permutations: Vec<permutation::prover::Committed<C>> = instance
        ...
        .zip(advice.iter())
        .zip(proof_instances.iter_mut())
        .map(|((instance, advice), instance_proof)| {
            pk.vk.cs.permutation.commit(
                ...
                gamma,
                transcript,
                &mut instance_proof.permutation,
            )
        })
        .collect::<Result<Vec<_>, _>>()?;
    ...
    let lookups: Vec<Vec<lookup::prover::Committed<C>>> = lookups
        .into_iter()
        .zip(proof_instances.iter_mut())
        .map(|(lookups, instance_proof)| -> Result<Vec<_>, _> {
            // Construct and commit to products for each lookup
            lookups
                .into_iter()
                .zip(instance_proof.lookups.iter_mut())
                .map(|(lookup, lookup_proof)| {
                    lookup.commit_product(pk, params, theta, beta, gamma, transcript, lookup_proof)
                })
                .collect::<Result<Vec<_>, _>>()
        })
        .collect::<Result<Vec<_>, _>>()?;
    ...
    let vanishing = vanishing::Argument::commit(params, domain, transcript, &mut vanishing_proof)?;
    ...
    for (instance, proof) in instance.iter().zip(proof_instances.iter_mut()) {
        ...
        proof.instance_evals = instance_evals.clone();
    ...
    for (advice, proof) in advice.iter().zip(proof_instances.iter_mut()) {
        // Evaluate polynomials at omega^i x
        ...
        proof.advice_evals = advice_evals.clone();
    ...
    proof.fixed_evals = fixed_evals.clone();
    ...
    let vanishing = vanishing.evaluate(x, xn, domain, transcript, &mut vanishing_proof)?;

    // Evaluate common permutation data
    pk.permutation.evaluate(x, transcript, &mut proof)?;

    // Evaluate the permutations, if any, at omega^i x.
    let permutations: Vec<permutation::prover::Evaluated<C>> = permutations
        .into_iter()
        .zip(proof_instances.iter_mut())
        .map(|(permutation, instance_proof)| -> Result<_, _> {
            permutation.evaluate(pk, x, transcript, &mut instance_proof.permutation)
        })
        .collect::<Result<Vec<_>, _>>()?;
    ...
    let lookups: Vec<Vec<lookup::prover::Evaluated<C>>> = lookups
        ...
        .map(|(lookups, instance_proof)| -> Result<Vec<_>, _> {
            lookups
                .into_iter()
                .zip(instance_proof.lookups.iter_mut())
                .map(|(lookup, lookup_proof)| lookup.evaluate(pk, x, transcript, lookup_proof))
                .collect::<Result<Vec<_>, _>>()
        })
        .collect::<Result<Vec<_>, _>>()?;
