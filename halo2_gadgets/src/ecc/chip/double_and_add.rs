...
impl Config {
    ...
    pub fn assign_region(
        ...
        p.point()
            .zip(q.point())
            .map(|(p, q)| {
                let q_dbl = q * pallas::Scalar::from(2);
                // P is point at infinity
                if p.to_curve().is_identity().into()
                // Q is point at infinity
                || q.to_curve().is_identity().into()
            // P = 2Q or P = -2Q
...
pub mod tests {
    ...
    #[test]
    pub fn test_double_and_add() {
        struct MyCircuit {
            p: Option<pallas::Affine>,
            q: Option<pallas::Affine>,
        }

        impl Circuit<pallas::Base> for MyCircuit {
            type Config = (super::Config, witness_point::Config);
