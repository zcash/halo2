    ...
    F: FieldExt + PrimeFieldBits,
{
    ...
    /// `z_0` must be the cell at `(self.z, offset + NUM_WINDOWS)` in `region`.
    ///
    /// # Panics
    ///
    ...
    fn decompose<const WORD_NUM_BITS: usize, const NUM_WINDOWS: usize>(
        ...
        for (i, word) in words.iter().enumerate() {
            let z_next = {
                let word: Option<F> = word.map(|word| F::from(word as u64));
                let z_next_val = z
                    .value()
                    .zip(word)
    ...
    fn test_running_sum_be() {
            ...
            > Circuit<F> for MyCircuit<F, WORD_NUM_BITS, WINDOW_NUM_BITS, NUM_WINDOWS>
        {
            ...
            fn configure(meta: &mut ConstraintSystem<F>) -> Self::Config {
                ...
                let config = RunningSumConfig::<F, WINDOW_NUM_BITS>::configure(meta, z);
