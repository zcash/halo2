...
impl<F: FieldExt, L: Layouter<Field = F>> CompressionConfig<'_, F, L> {
    ...
    pub(super) fn decompose_a(&mut self, idx: i32, a_val: u32) -> Result<RoundWordA, Error> {
        let row = get_decompose_a_row(idx);

        let (dense_halves, spread_halves) = self.assign_word_halves(row, a_val)?;
        let (a, b, c_lo, c_mid, c_hi, d) = self.decompose_abcd(row, a_val)?;
    ...
    pub(super) fn decompose_e(&mut self, idx: i32, e_val: u32) -> Result<RoundWordE, Error> {
        let row = get_decompose_e_row(idx);

        let (dense_halves, spread_halves) = self.assign_word_halves(row, e_val)?;
        let (a_lo, a_hi, b_lo, b_hi, c, d) = self.decompose_efgh(row, e_val)?;
    ...
    pub(super) fn assign_upper_sigma_0(
        ...
        self.assign_and_constrain(|| "spread_b", a_5, row, &word.b.spread, &configured.perm)?;
        ...
        self.assign_and_constrain(|| "spread_d", a_4, row, &word.d.spread, &configured.perm)?;
    ...
    pub(super) fn assign_upper_sigma_1(
        ...
        self.assign_and_constrain(|| "spread_c", a_5, row, &word.c.spread, &configured.perm)?;
        // Assign `spread_d` and copy constraint
        self.assign_and_constrain(|| "spread_d", a_4, row, &word.d.spread, &configured.perm)?;
    ...
    pub(super) fn assign_ch(
        ...
        self.assign_ch_outputs(row, p0_even, p0_odd, p1_even, p1_odd)
    }
    ...
    pub(super) fn assign_maj(
        ...
        self.assign_maj_outputs(row, m0_even, m0_odd, m1_even, m1_odd)
    }
    ...
    pub(super) fn assign_h_prime(
        ...
        // Assign and copy h
        self.assign_and_constrain(|| "h_lo", a_7, row - 1, &h.0.into(), &configured.perm)?;
        self.assign_and_constrain(|| "h_hi", a_7, row, &h.1.into(), &configured.perm)?;
        ...
        // Assign and copy w
        self.assign_and_constrain(|| "w_lo", a_8, row - 1, &w.0.into(), &configured.perm)?;
        self.assign_and_constrain(|| "w_hi", a_8, row, &w.1.into(), &configured.perm)?;

        // Assign and copy ch
        self.assign_and_constrain(|| "ch_neg_hi", a_6, row + 1, &ch.1.into(), &configured.perm)?;
    ...
    pub(super) fn assign_e_new(
        ...
        self.assign_and_constrain(|| "d_lo", a_7, row, &d.0.into(), &configured.perm)?;
        self.assign_and_constrain(|| "d_hi", a_7, row + 1, &d.1.into(), &configured.perm)?;
        ...
        let e_new_dense = self.assign_word_halves_dense(row, a_8, row + 1, a_8, e_new)?;
    ...
    pub(super) fn assign_a_new(
        ...
        self.assign_and_constrain(|| "maj_1_hi", a_3, row - 1, &maj.1.into(), &configured.perm)?;
        ...
        let a_new_dense = self.assign_word_halves_dense(row, a_8, row + 1, a_8, a_new)?;
    ...
    pub fn assign_word_halves(
        ...
        self.layouter().assign_region(
            ...
            |mut region: Region<'_, Self>| {
                ...
                let spread_w_lo_cell =
                    CellValue32::new(w_lo.spread.var, w_lo.spread.value.unwrap());
                let spread_w_hi_cell =
                    CellValue32::new(w_hi.spread.var, w_hi.spread.value.unwrap());
