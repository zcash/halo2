...
impl<F: FieldExt, L: Layouter<Field = F>> CompressionConfig<'_, F, L> {
    ...
    pub fn assign_round(
        ...
        // s_upper_sigma_1(E)
        let sigma_1 = self.assign_upper_sigma_1(idx, e.pieces.unwrap())?;

        // Ch(E, F, G)
        let ch = self.assign_ch(idx, e.spread_halves.unwrap(), f.spread_halves)?;
        let ch_neg = self.assign_ch_neg(idx, e.spread_halves.unwrap(), g.spread_halves)?;

        // s_upper_sigma_0(A)
        let sigma_0 = self.assign_upper_sigma_0(idx, a.pieces.unwrap())?;
        ...
        // E_new = H' + D
        let e_new_dense = self.assign_e_new(idx, d.dense_halves, h_prime)?;
        let e_new_val = val_from_dense_halves(e_new_dense);

        // A_new = H' + Maj(A, B, C) + sigma_0(A)
        let a_new_dense = self.assign_a_new(idx, maj, sigma_0, h_prime)?;
        ...
        if idx < 63 {
            ...
            let a_new = self.decompose_a(idx + 1, a_new_val)?;

            // Decompose E into (6, 5, 14, 7)-bit chunks
            let e_new = self.decompose_e(idx + 1, e_new_val)?;
            ...
            let a_final = self.assign_word_halves_dense(abcd_row, a_3, abcd_row, a_4, a_new_val)?;

            let e_final = self.assign_word_halves_dense(efgh_row, a_3, efgh_row, a_4, e_new_val)?;
