...
impl<F: FieldExt, L: Layouter<Field = F>> MessageScheduleConfig<'_, F, L> {
    ...
    pub fn assign_subregion2(
        ...
        let mut new_word = |idx: usize,
         ...
         -> Result<Vec<(CellValue16, CellValue16)>, Error> {
            ...
            let subregion2_word = self.decompose_subregion2_word(w[idx].value.unwrap(), idx)?;

            // sigma_0 v2 and sigma_1 v2 on subregion2_word
            lower_sigma_0_v2_results.push(self.lower_sigma_0_v2(subregion2_word.clone())?);
            lower_sigma_1_v2_results.push(self.lower_sigma_1_v2(subregion2_word)?);
    ...
    fn decompose_subregion2_word(
        ...
        self.layouter().assign_region(
            ...
            |mut region: Region<'_, Self>| {
                ...
                let spread_b =
                    SpreadVar::with_lookup(&mut region, &configured.lookup, row + 1, spread_b)?;
                ...
                let spread_g =
                    SpreadVar::with_lookup(&mut region, &configured.lookup, row - 1, spread_g)?;
    ...
    fn assign_lower_sigma_v2_pieces(
        ...
        self.assign_and_constrain(|| "a", a_3, row + 1, &subregion2_word.a, &configured.perm)?;

        // Assign `b` and copy constraint
        self.assign_and_constrain(|| "b", a_6, row, &subregion2_word.b, &configured.perm)?;

        // Assign `c` and copy constraint
        self.assign_and_constrain(|| "c", a_5, row + 1, &subregion2_word.c, &configured.perm)?;
        ...
        self.assign_and_constrain(|| "e", a_7, row, &subregion2_word.e, &configured.perm)?;

        // Assign `f` and copy constraint
        self.assign_and_constrain(|| "f", a_7, row + 1, &subregion2_word.f, &configured.perm)?;
    ...
    fn lower_sigma_0_v2(
        ...
        let (spread_a, spread_b_lo, spread_b_hi, spread_c, spread_d, e, f, spread_g) =
            self.assign_lower_sigma_v2_pieces(row, subregion2_word)?;
