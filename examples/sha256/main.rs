...
pub use table16::{BlockWord, Table16Chip, Table16Config, Table16Core};
...
impl<Sha256Core: Sha256Instructions> Sha256<Sha256Core> {
    /// Create a new hasher instance.
    pub fn new(core: &mut Sha256Core) -> Result<Self, Error> {
        let state = core.initialization_vector()?;
        Ok(Sha256 {
            // config,
            state,
    ...
    pub fn update(
        ...
        for chunk in &mut chunks_iter {
            self.state = core.initialization(&self.state)?;
            self.state = core.compress(
                &self.state,
    ...
    pub fn finalize(
        &mut self,
        core: &mut Sha256Core,
    ) -> Result<Sha256Digest<Sha256Core::BlockWord>, Error> {
        // Pad the remaining block
        if !self.cur_block.is_empty() {
            let padding = vec![Sha256Core::zero(); BLOCK_SIZE - self.cur_block.len()];
        ...
        core.digest(&self.state).map(Sha256Digest)
    }
    ...
    pub fn digest(
        core: &mut Sha256Core,
        data: &[Sha256Core::BlockWord],
    ) -> Result<Sha256Digest<Sha256Core::BlockWord>, Error> {
        let mut hasher = Self::new(core)?;
        hasher.update(core, data)?;
        hasher.finalize(core)
    }
}
