...
// ANCHOR: config
/// The config that will implement our instructions!
...
// ANCHOR: configure
/// Config state is stored in a separate `Configured` struct. This is generated by the
/// config during configuration, and made available to the `Layouter` when assigning
/// values in the circuit.
#[derive(Clone, Debug)]
struct FieldConfigured {
    /// For this config, we will use two advice columns to implement our instructions.
...
impl<F: FieldExt> Circuit<F> for MyCircuit<F> {
    // Since we are using a single config for everything, we can just reuse its configured state.
    type Configured = FieldConfigured;
