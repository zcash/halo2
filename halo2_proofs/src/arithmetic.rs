...
fn swap_arithmetic<G: Group>(a: &mut G, b: &mut G) {
    let t = *b;
    *b = *a;
    a.group_add(&t);
    b.group_sub(&t);
...
fn swap_arithmetic_with_twiddle<G: Group>(a: &mut G, b: &mut G, twiddle: &G::Scalar) {
    let mut t = *b;
...
/// This performs permutation and butterfly arithmetic without twiddle for `[G]`
fn self_swap_arithmetic<G: Group>(a: &mut [G], b: &mut [G], former: usize, latter: usize) {
    let t = a[latter];
    a[latter] = a[former];
...
/// This performs permutation and butterfly arithmetic twiddle for `[G]`
fn self_swap_arithmetic_with_twiddle<G: Group>(
    a: &mut [G],
    b: &mut [G],
    twiddle: &G::Scalar,
    former: usize,
