...
`halo2_core::dev::MockProver` is a tool for debugging circuits, as well as cheaply verifying
...
`halo2_core::dev::circuit_layout` renders the circuit layout as a grid:
...
`halo2_core::dev::circuit_dot_graph` builds a [DOT graph string] representing the given
