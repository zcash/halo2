...
use halo2_core::arithmetic::FieldExt;
use halo2_core::circuit::{Cell, Layouter, SimpleFloorPlanner};
use halo2_core::dev::MockProver;
use halo2_core::pasta::{Eq, EqAffine, Fp};
use halo2_core::plonk::{
    create_proof, keygen_pk, keygen_vk, verify_proof, Advice, Circuit, Column, ConstraintSystem,
    Error, Fixed, TableColumn, VerifyingKey,
};
use halo2_core::poly::{commitment::Params, Rotation};
use halo2_core::transcript::{Blake2bRead, Blake2bWrite, Challenge255};
...
fn plonk_api() {
    ...
    for _ in 0..10 {
        ...
        assert_eq!(
            proof.len(),
            halo2_core::dev::CircuitCost::<Eq, MyCircuit<_>>::measure(K as usize, &circuit)
                .proof_size(2)
                .into(),
        );
